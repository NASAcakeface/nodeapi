...
var uploadFile = function uploadImage(req, res, next) {
  var accessKey = '0FRsuC+m7ygXy6R2+4O9I8W3cZRC2hR5H7xn/Ic/Pw8FqbR7Z/jmJM7vE57SRlBFvDGWBiUXaTFa8W0hAJx3lA==';
  var storageAccount = 'nasastorage';
  ...
  if(!req.files.item.name){
    res.send(404, "Item must be a file");
    return next();
  }
  ...
  var timestampInt = Date.now()
  var filename = user_id + "_" + timestampInt + "_" + item.name;
  ...
  blobService.createBlockBlobFromStream(container, filename, stream, item.size, function (error) {
    ...
    if (error) {
      ...
      var url = "https://nasastorage.blob.core.windows.net/" + container + "/" + filename;
      if(container == 'image'){
  ...
  streamWav.on('finish', function(){
    ...
    speech_to_text.recognize(params, function(err, transcript) {
      ...
      if (err){
        ...
        if(transcript.results.length < 1) {
          return callback("Watson could not generate text from input");
        }
...
var sendRequest = function sendRequest(url, user_id, time, type, data, callback){
  ...
  var options = {
    uri: 'http://192.241.242.189/emotions/',
    method: 'POST',
    json: {
      "url": url,
      "user_name": user_id,
